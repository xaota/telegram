...
const buildUploadPhotoObject = R.pipe(R.set(R.lensProp('file'), R.__, {}));
...
function signUpWithoutAvatar(connection, data) {
  return of(data).pipe(
    map(methodSignUp),
    switchMap(x => fromPromise(connection.request(x)).pipe(catchError(R.of)))
  );
}
...
function uploadAvatar(connection, avatar) {
  ...
  return fromPromise(promise)
    .pipe(
      ...
      switchMap(x => fromPromise(connection.request(x))),
      catchError(R.of)
    );
}
